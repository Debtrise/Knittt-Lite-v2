      ...
      res.on('end', () => {
        try {
          ...
          const result = {
            ...
            data: jsonData,
            error: res.statusCode >= 400 ? jsonData.error || 'Unknown error' : null
          };
          
          console.log(`\n🔄 ${method} ${endpoint}`);
          if (result.success) {
            console.log(`✅ Success (${res.statusCode}):`, JSON.stringify(jsonData, null, 2));
          } else {
            console.log(`❌ Failed (${res.statusCode}):`, result.error);
          }
