...
export default function JourneyLeadsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchLeads = async () => {
    try {
      setIsLoading(true);
      const response = await getJourneyLeads(journeyId, {
  ...
  const handleEnrollLeads = async (leadIds: number[]) => {
    try {
      setIsEnrolling(true);
      await enrollLeadsInJourney(journeyId, { leadIds });
  ...
  const handleUpdateLeadStatus = async (leadId: number, status: string) => {
    try {
      await updateLeadJourneyStatus(leadId, journeyId, { status });
  ...
  const handleBulkAction = async (action: string) => {
    ...
    try {
      ...
      for (const leadId of selectedLeads) {
        await updateLeadJourneyStatus(leadId, journeyId, { status: action });
      }
